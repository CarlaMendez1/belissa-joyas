...
import { getCategorias, getSubcategoriasPorCategoria } from '@/lib/api';
...
export default function AdminCategoriasPage() {
  ...
  const [confirmando, setConfirmando] = useState<
    { tipo: 'categoria'; item: any } | { tipo: 'subcategoria'; id_categoria: number; item: any } | null
  >(null);
  ...
  function pedirEliminarCategoria(categoria: any) {
    setConfirmando({ tipo: 'categoria', item: categoria });
  }

  function pedirEliminarSub(id_categoria: number, subcat: any) {
    setConfirmando({ tipo: 'subcategoria', id_categoria, item: subcat });
  }

  async function confirmarEliminacion() {
    if (!confirmando) return;
    try {
      if (confirmando.tipo === 'categoria') {
        await eliminarCategoria(token, confirmando.item.id_categoria);
        await cargar();
      } else {
        await eliminarSubcategoria(token, confirmando.item.id_subcategoria);
        await recargarSubcats(confirmando.id_categoria);
      }
    } catch (err: any) {
  ...
  return (
    <div>
            ...
            <div key={cat.id_categoria} className="bg-white border border-stone-200 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => pedirEliminarCategoria(cat)} className="text-stone-400 hover:text-red-500">
                      ...
                      {subcats[cat.id_categoria].map((sub: any) => (
                        ...
                        >
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => pedirEliminarSub(cat.id_categoria, sub)}
      ...
      <ConfirmDialog
        abierto={!!confirmando}
        titulo={confirmando?.tipo === 'categoria' ? 'Eliminar categoría' : 'Eliminar subcategoría'}
        mensaje={
          confirmando?.tipo === 'categoria'
            ? `¿Eliminar la categoría "${confirmando.item.nombre}"?`
            : `¿Eliminar la subcategoría "${confirmando?.item.nombre}"?`
        }
